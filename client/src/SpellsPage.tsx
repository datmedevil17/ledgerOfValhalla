...
function CastEffect({ e }: { e: ActiveEffect }) {
  ...
  switch (e.spellId) {
    ...
    case 'fireball':
      return <div style={at({})}>
        <div style={{ position:'absolute', width:e.r*2, height:e.r*2, borderRadius:'50%',
          background:`radial-gradient(circle, ${e.color}cc 0%, ${e.color}66 45%, transparent 75%)`,
          border:`3px solid ${e.color}`, transform:'translate(-50%,-50%)',
          animation:'fx-expand-fire 1.1s ease-out forwards',
          boxShadow:`0 0 40px ${e.color}88` }}/>
        {[0,40,80,120,160,200,240,300].map((deg,i) => {
          const rad = deg*Math.PI/180, dist = e.r*(1.1+rnd(e.seed,i)*.35)
          return <div key={i} style={{ position:'absolute', width:10, height:10,
            borderRadius:'50%', background: i%2?'#ffcc00':'#ff8800',
            transform:'translate(-50%,-50%)',
            ['--dx' as any]:`${Math.cos(rad)*dist}px`,
            ['--dy' as any]:`${Math.sin(rad)*dist}px`,
            animation:`fx-spark ${.7+rnd(e.seed,i+10)*.35}s ease-out ${i*.03}s forwards`}}/>
        })}
        <div style={{ position:'absolute', top:-14, left:0, transform:'translate(-50%,0)',
          color:'#ff6600', fontWeight:900, fontSize:22, textShadow:'0 0 12px #ff2200',
          animation:'fx-float 1.1s ease-out .15s forwards', opacity:0 }}>-200</div>
      </div>
    ...
    case 'element-fire':
      return <div style={at({})}>
        ...
        {Array.from({length:12}).map((_,i) => {
          const ang = rnd(e.seed,i)*Math.PI*2, r2 = e.r*(.2+rnd(e.seed,i+20)*.75)
          return <div key={i} style={{ position:'absolute',
            left:Math.cos(ang)*r2, top:Math.sin(ang)*r2,
            transform:'translate(-50%,-50%)', fontSize:20+Math.round(rnd(e.seed,i+40)*12),
        ...
        <div style={{ position:'absolute', top:-14, left:0, transform:'translate(-50%,0)',
          ...
          animation:'fx-float 1.8s ease-out .1s forwards', opacity:0 }}>🔥 BURNING 30 DPS · 5s</div>
      </div>
      ...
      return <div style={at({})}>
        ...
        {[0,60,120,180,240,300].map((deg,i) => {
          ...
          return <div key={i} style={{ position:'absolute',
            ...
            animation:`fx-spark 1.1s ease-out ${i*.07}s forwards` }}>→</div>
        })}
        <div style={{ position:'absolute', top:-14, left:0, transform:'translate(-50%,0)',
          color:'#aaddff', fontWeight:800, fontSize:15,
          animation:'fx-float 1.4s ease-out .1s forwards', opacity:0 }}>💨 PUSH — 8 units</div>
      </div>

    case 'element-dark':
      return <div style={at({})}>
        <div style={{ position:'absolute', width:e.r*2, height:e.r*2,
          background:`${e.color}e8`, border:`2px solid ${e.color}`,
          transform:'translate(-50%,-50%)',
          animation:'fx-dark 3s ease-out forwards',
          boxShadow:`0 0 60px ${e.color}88` }}>
          {[25,50,75].map((_,i) => (
            <div key={i} style={{ position:'absolute',
              left:`${15+i*30}%`, top:`${30+i*15}%`,
              transform:'translate(-50%,-50%)', fontSize:26,
              animation:`fx-emerge .5s ease-out ${.2+i*.18}s forwards`, opacity:0 }}>🚫</div>
          ))}
        </div>
        <div style={{ position:'absolute', top:-14, left:0, transform:'translate(-50%,0)',
          color:'#cc44ff', fontWeight:800, fontSize:15,
          animation:'fx-float 1s ease-out .2s forwards', opacity:0 }}>🌑 BLIND · 8s</div>
      </div>

    case 'element-lightning':
      return <div style={at({})}>
        <div style={{ position:'absolute', width:5, height:90, borderRadius:3,
          background:`linear-gradient(to bottom, transparent, ${e.color}, transparent)`,
          transform:'translate(-50%,-100%)',
          animation:'fx-bolt .7s ease-out forwards',
        ...
        <div style={{ position:'absolute', width:36, height:36, borderRadius:'50%',
          ...
          animation:'fx-expand .5s ease-out .05s forwards',
          boxShadow:`0 0 24px ${e.color}` }}/>
        {[[-90,40],[65,-28]].map(([dx,dy],i) => (
          <div key={i} style={{ position:'absolute',
            width:Math.hypot(dx,dy), height:4, borderRadius:2,
            ...
            animation:`fx-chain .7s ease-out ${.12+i*.09}s forwards`,
            opacity:0, boxShadow:`0 0 7px ${e.color}` }}/>
        ))}
        <div style={{ position:'absolute', top:-18, left:0, transform:'translate(-50%,0)',
          color:e.color, fontWeight:900, fontSize:20,
          animation:'fx-float 1.1s ease-out .3s forwards', opacity:0 }}>⚡ -400 → chain -150</div>
      </div>

    case 'trenchcoat':
      return <div style={at({})}>
        {[[-24,0],[24,0],[0,-24],[-16,18],[16,18]].map(([dx,dy],i) => (
          <div key={i} style={{ position:'absolute', left:dx, top:dy,
            transform:'translate(-50%,-50%)', fontSize:26,
            animation:`fx-mush-reveal 3.2s ease-in-out forwards`,
            animationDelay:`${i*.08+2.3}s` }}>🍄</div>
        ))}
        <div style={{ position:'absolute', width:70, height:70, borderRadius:'50%',
          border:`2px dashed #44cc44`, transform:'translate(-50%,-50%)',
          animation:'reticle-pulse .8s ease-in-out 3 forwards' }}/>
        <div style={{ position:'absolute', top:-18, left:0, transform:'translate(-50%,0)',
          color:'#44cc44', fontWeight:800, fontSize:14,
          animation:'fx-float 1s ease-out 2.8s forwards', opacity:0 }}>👁 REVEALED</div>
      </div>

    case 'monk':
    case 'wizard':
    case 'lightning-wizard': {
      const isMonk = e.spellId==='monk'
      const isLW   = e.spellId==='lightning-wizard'
      const offsets = isMonk ? [[-28,0],[0,-20],[28,0]] : [[0,0]]
      return <div style={at({})}>
        ...
        {offsets.map(([dx,dy],i) => (
          <div key={i} style={{ position:'absolute', left:dx, top:dy,
            transform:'translate(-50%,-50%)', fontSize: isMonk?22:30,
            filter:`drop-shadow(0 0 8px ${e.color})`,
            animation:`fx-emerge .7s ease-out ${.3+i*.18}s forwards`, opacity:0 }}>
            {isMonk?'🥋':isLW?'⚡🔮':'🔮'}
          </div>
        ))}
        {isLW && <div style={{ position:'absolute', top:-18, left:0,
          transform:'translate(-50%,0)', color:e.color, fontWeight:800, fontSize:14,
          animation:'fx-float 1s ease-out .5s forwards', opacity:0 }}>2× STATS · 30s</div>}
      </div>
    }
    ...
    case 'steal':
      return <div style={at({})}>
        {[0,1,2].map(i => (
          <div key={i} style={{ position:'absolute', fontSize:28,
            transform:'translate(-50%,-50%)',
            animation:`fx-coin-fly 1s ease-in ${i*.13}s forwards` }}>🪙</div>
        ))}
        <div style={{ position:'absolute', top:-18, left:0, transform:'translate(-50%,0)',
          color:'#ffdd00', fontWeight:900, fontSize:20, textShadow:'0 0 10px #ff8800',
          animation:'fx-float 1.1s ease-out .4s forwards', opacity:0 }}>+300 💰</div>
      </div>
      ...
      return <div style={at({})}>
        {[[-40,0],[0,-28],[40,0]].map(([dx,dy],i) => (
          <div key={i} style={{ position:'absolute', left:dx, top:dy,
            ...
            animation:`fx-ghost 2s ease-out ${i*.22}s forwards`, opacity:0 }}>👻</div>
        ))}
        <div style={{ position:'absolute', top:-22, left:0, transform:'translate(-50%,0)',
          color:e.color, fontWeight:800, fontSize:15,
          animation:'fx-float 1.1s ease-out .8s forwards', opacity:0 }}>✨ REVIVED ×3 · 50% HP</div>
      </div>
    ...
    case 'market':
      return <div style={at({})}>
        ...
        <div style={{ position:'absolute', top:-30, left:0, transform:'translate(-50%,0)',
          color:'#ffaa00', fontWeight:900, fontSize:20, textShadow:'0 0 10px #ff8800',
          animation:'fx-float 1.6s ease-out .2s forwards', opacity:0 }}>+800💰 +200🌾</div>
      </div>

    case 'coin':
      return <div style={at({})}>
        <div style={{ position:'absolute', fontSize:52,
          transform:'translate(-50%,-50%)',
          animation:'fx-coin-fly 1.3s ease-in-out forwards' }}>🪙</div>
        <div style={{ position:'absolute', top:-22, left:0, transform:'translate(-50%,0)',
          color:'#ffdd00', fontWeight:900, fontSize:22, textShadow:'0 0 10px #ffaa00',
          animation:'fx-float 1.3s ease-out .5s forwards', opacity:0 }}>+500 💰</div>
      </div>

    case 'polinization':
      return <div style={{ position:'fixed', left:0, top:e.y, width:'100vw', height:0,
        pointerEvents:'none' }}>
        ...
        <div style={{ position:'absolute', left:'50%', top:-40, transform:'translate(-50%,0)',
          color:'#88ff44', fontWeight:800, fontSize:18,
          animation:'fx-float 1s ease-out .4s forwards', opacity:0 }}>🌸 3× FARM · 2 Hours</div>
      </div>
    ...
    case 'ocean-treasure': {
      ...
      return <div style={at({})}>
        <div style={{ position:'absolute', fontSize:46,
          transform:'translate(-50%,-50%)',
          animation:'fx-chest 2.8s ease-out forwards' }}>🎁</div>
        {Array.from({length:8}).map((_,i) => (
          <div key={i} style={{ position:'absolute', fontSize:18,
            transform:'translate(-50%,-50%)',
            ['--dx' as any]:`${(rnd(e.seed,i+10)-.5)*100}px`,
            ['--dy' as any]:`${-25-rnd(e.seed,i+20)*50}px`,
            animation:`fx-spark 1.1s ease-out ${.6+i*.1}s forwards` }}>🪙</div>
        ))}
        <div style={{ position:'absolute', top:-28, left:0, transform:'translate(-50%,0)',
          color:'#00aaff', fontWeight:900, fontSize:22, textShadow:'0 0 10px #0088ff',
          animation:'fx-result 2.2s ease-out .9s forwards', opacity:0 }}>{prize}</div>
      </div>
    }

    case 'book':
      return <div style={at({})}>
        <div style={{ position:'absolute', fontSize:44,
          transform:'translate(-50%,-50%)',
          animation:'fx-book 1.7s ease-out forwards' }}>📚</div>
        <div style={{ position:'absolute', top:-26, left:0, transform:'translate(-50%,0)',
          color:'#ddaa44', fontWeight:900, fontSize:20, textShadow:'0 0 8px #aa7700',
          animation:'fx-float 1.4s ease-out .35s forwards', opacity:0 }}>⏩ −30 min</div>
      </div>
    ...
    case 'roll-dice': {
      ...
      return <div style={at({})}>
        <div style={{ position:'absolute', fontSize:52,
          transform:'translate(-50%,-50%)',
          animation:'fx-dice 1.7s ease-out forwards' }}>🎲</div>
        <div style={{ position:'absolute', top:-30, left:0, transform:'translate(-50%,0)',
          color:o.color, fontWeight:900, fontSize:20, textShadow:`0 0 10px ${o.color}`,
          animation:'fx-result 2s ease-out 1.3s forwards', opacity:0 }}>{o.label}</div>
      </div>
    }
...
function Reticle({ spell, x, y }: { spell: SpellDef; x: number; y: number }) {
  ...
  if (spell.targetMode === 'aoe-circle') {
    const d = r * 2
    return <>
      <div style={{ ...base, left:x-r, top:y-r, width:d, height:d, borderRadius:'50%',
        border:`2px solid ${spell.color}`, background:`${spell.color}18`,
        boxShadow:`0 0 20px ${spell.color}66` }}/>
      {/* Expanding ring pulse */}
      <div style={{ position:'fixed', left:x-r, top:y-r, width:d, height:d,
  ...
  if (spell.targetMode === 'aoe-square') {
    const side = r * 1.42
    return <div style={{ ...base, left:x-side/2, top:y-side/2, width:side, height:side,
      border:`2px solid ${spell.color}`, background:`${spell.color}18`,
      boxShadow:`0 0 20px ${spell.color}66` }}/>
  }
  ...
  if (spell.targetMode === 'point') {
    const arm = 28
    return <>
      <div style={{ position:'fixed', left:x-arm, top:y-1, width:arm*2, height:2,
        background:`${spell.color}cc`, pointerEvents:'none' }}/>
      <div style={{ position:'fixed', left:x-1, top:y-arm, width:2, height:arm*2,
        background:`${spell.color}cc`, pointerEvents:'none' }}/>
      <div style={{ ...base, left:x-10, top:y-10, width:20, height:20, borderRadius:'50%',
...
function SpellInfoPanel({ spell, onCancel }: { spell: SpellDef; onCancel: () => void }) {
  ...
  return (
    <div style={{ position:'fixed', top:16, left:16, zIndex:30, width:280,
      ...
      <div style={{ padding:'12px 14px', display:'flex', gap:12 }}>
        {/* Art */}
        <div style={{ width:68, height:86, borderRadius:8, flexShrink:0, overflow:'hidden',
        ...
        <div style={{ flex:1, display:'flex', flexDirection:'column', gap:5 }}>
          ...
          <div style={{ display:'flex', gap:5, flexWrap:'wrap' }}>
            <span style={{ background:tierColor, color:'#000', fontSize:9,
              fontWeight:900, borderRadius:3, padding:'1px 5px' }}>{tierLabel}</span>
            <span style={{ background: spell.ctx==='battle'?'#4a0011':'#003311',
      ...
      <div style={{ padding:'0 14px 10px' }}>
        ...
        <div style={{ padding:'6px 8px', background:`${spell.color}18`, borderRadius:6,
      ...
      <div style={{ width:'100%', height:'100%', borderRadius:9, overflow:'hidden',
        ...
        {spell.img
          ...
          : <>
              ...
              <span style={{ fontSize:7.5, color:'#ccc', textAlign:'center',
                padding:'0 4px', lineHeight:1.2 }}>{spell.name}</span>
            </>
        }
      </div>
    ...
    <div style={{ position:'fixed', bottom:0, left:0, right:0, zIndex:20,
      ...
      <div style={{ display:'flex', alignItems:'flex-start', gap:20 }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
          <div style={{ fontSize:9, fontWeight:700, color:'#ff6644', letterSpacing:1.5,
            textTransform:'uppercase', paddingLeft:2 }}>⚔️ Battle</div>
        ...
        {/* Home spells */}
        <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
          <div style={{ fontSize:9, fontWeight:700, color:'#66ff88', letterSpacing:1.5,
            textTransform:'uppercase', paddingLeft:2 }}>🏠 Home</div>
        ...
        >
          ...
          {selected?.targetMode === 'auto' && (
            <div style={{ position:'fixed', top:'50%', left:'50%',
              ...
              <div style={{ fontSize:14, color:`${selected.color}cc`,
